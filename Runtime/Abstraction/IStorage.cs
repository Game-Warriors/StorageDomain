...
namespace GameWarriors.StorageDomain.Abstraction
{
    public interface IStorage
    {
        string FileRoot { get; }
        byte[] Key { get; }
        Task<T> RegisterLoadingModel<T>(string dataName, bool isEncrypt) where T : IStorageItem ,new();
        Task<U> RegisterLoadingDefaultModel<T, U>(string dataName, bool isEncrypt) where T : IStorageDataItem, new() where U : IDefaultDataModel, new();
